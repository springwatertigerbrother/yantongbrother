...
//     CGPoint origin = CGPointMake(0.0,CGSizeFromVponAdSize(VponAdSizeBanner).height);
    VpadnBanner *vpon = [[vponBannerClass alloc] initWithAdSize:VpadnAdSizeBanner];//sharedInstance] adwhirlRequestDelegate:self
//																  licenseKey:[self adonLicenseKey] 
//																		size:self.sSizeAd];
    vpon.strBannerId = [self adonLicenseKey];
    vpon.delegate = self;
    vpon.platform = @"CN";
    [vpon setIsAccessibilityElement:YES];
    [vpon setAdAutoRefresh:NO];
    [vpon setLocationOnOff:[self helperUseGpsMode]];
    [vpon setRootViewController:[self.adViewView.delegate viewControllerForPresentingModalView]];
    
    if (nil == vpon) {
        [adViewView adapter:self didFailAd:nil];
        return;
    }
    UIView *adView = [vpon getVpadnAdView];
    [vpon startGetAd:[self getTestIdentifiers]];
...
{
    return [NSArray arrayWithObjects:@"E42D4D04-E927-4B9D-93DF-84762823CD34",@"36FA0EC8-B46E-45BA-85D0-E05F2D6C9EBF",
            // add your test UUID
            nil];
}
